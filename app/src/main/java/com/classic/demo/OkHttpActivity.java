...
import com.classic.android.http.observer.SimpleObserver;
...
public class OkHttpActivity extends RxActivity {
    ...
    private void testFaceApi(@NonNull String imagePath1, @NonNull String imagePath2){
        mFaceApi.compare(convert(PrivateConstant.FACE_API_ID),
                ...
                .subscribeWith(new SimpleObserver<IdentifyResult>() {
                    @Override public void onNext(IdentifyResult result) {
                        XLog.d("FaceApi --> " + result.toString());
                    }

                    @Override public void onError(Throwable throwable) {
                        super.onError(throwable);
                        XLog.e("FaceApi --> " + throwable.getMessage());
                    }
                });
    }
    ...
    private static final Function<String, IdentifyResult> DATA_PARSE_FUNCTION =
        ...
        @Override public IdentifyResult apply(String s) throws Exception {
            if(null != s){
